...
token = '2095089240:AAEE6mOEO42cRkshN9kTTS4MOJ00pL9v-sI'
...
def profile(message):
    bot.send_message(message.chat.id, "Thank you! I received your information")
    conn = sqlite3.connect("users.db")
    cursor = conn.cursor()
    f = cursor.execute(f"SELECT * FROM main WHERE chat_id = {str(message.chat.id)}").fetchone()
    if f ==None:
        cursor.execute(f"insert into main values ({str(message.chat.id)}, '{message.text}', 'None')")
    else:
...
def text(message):
    ...
    if cou != None:
        if cou >= 5:
            ...
            bot.send_message(message.chat.id, "If you are feeling unwell, you can visit this website.: https://www.pflege.de/")
...
def ask(question, message,chat_log=None):
 ...
 lf = cursor.execute(f"SELECT last_promt FROM main WHERE chat_id = {str(message.chat.id)}").fetchone()[0]
 if lf != 'None':
    prompt_text = f'{session_prompt}\n\n{lf}{restart_sequence}{question}{start_sequence}'
 else:
 ...
 cursor.execute(f"UPDATE main SET last_promt = 'Person: {message.text}\nBot: {story}' WHERE chat_id = {str(message.chat.id)}")
 conn.commit()
 conn.close()
 return str(story)
...
def callback_register(call):
    if call.data == 'need_care':
        ...
        degrees_increased(call.message)
    elif call.data == 'benefits':
        benefits(call.message)
    elif call.data == 'sa':
        sa(call.message)
    elif call.data == 'Apply for care degree':
        pflegegrad(call.message)
...
def degrees_increased(message):
    text = "First check wheather you are using all the benefits of your degree. Otherwise you can change the degree"
    bot.send_message(message.chat.id, text)

def benefits(message):
    text = "Family caregivers are entitled to care courses, care leave, family care leave, care support allowance and rehabilitation. At the request of the person in need of care, family caregivers are entitled to care counseling."
    bot.send_message(message.chat.id, text)
def sa(message):
    text = "The relief amount is an earmarked benefit in kind of 125 euros for people in need of care who live in a home environment. The relief amount can be used to claim care to assist with household chores or the organization of everyday care. Offers for support in everyday life must be approved by the respective state law."
    bot.send_message(message.chat.id, text)
